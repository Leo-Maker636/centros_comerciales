...
def delimitar_busqueda_establecimientos(ruta_base_rucs_sri: Path) -> pl.DataFrame:
    ...
    base_rucs_cerca: pl.DataFrame = (
        ...
        .with_columns(
            ...
            .str.contains(
                r"(?i)^\s*GUAYAS\s*/\s*GUAYAQUIL\s*/\s*IÑAQUITO",
...
def encontrar_posibles_locales_jardin(threshold: float):
    ...
    mallsol_nombres = pl.read_csv(
        "../tablas_locales_CC/Mall_del_Sol_locales.tsv", separator="\t"
    )
    set_nombre_fantasia = set(mallsol_nombres["LOCAL"])
    set_nombre_fantasia_normalizado: set = {
        _normalizar(local) for local in set_nombre_fantasia
    }

    print(
        f"Se tienen {len(set_nombre_fantasia_normalizado)} locales registrados en el CC: MallElSol."
    )
